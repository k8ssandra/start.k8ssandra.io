<template>
    <div>
        <h5>Medusa Google Cloud Storage</h5>
        <div>
            <label>Name</label><br />
...
export default {
  ...
  computed: {
    name: {
      get() {
        return this.$store.state.settings.k8_config.medusa.provider_gcs.config.bucket_name;
      },
      set(value) {
    ...
    storage_secret: {
      get() {
        return this.$store.state.settings.k8_config.medusa.provider_gcs.config.secret;
      },
      set(value) {
